...
*Rendered diagram: `docs/diagrams/data_flow.png` (PNG for reports/slides) and `docs/diagrams/data_flow.svg` (scalable vector). The Mermaid source below renders natively on GitHub.*
...
sequenceDiagram
    ...
    BE->>AI: Send prompt (HTTPS, API key from SSM Parameter Store)
...
10. **AI API call.** The backend calls the external AI provider over HTTPS using an API key fetched from SSM Parameter Store under the instance's IAM role. The key never reaches the frontend or the repository.
...
| Credentials, API keys | Operator | SSM Parameter Store (SecureString) | Never in code, logs, or the database |

## 5. Security-Relevant Observations for Week 5

Validation happens twice by design (client and server), with the server as the authority. The AI API key exists only in SSM Parameter Store, fetched server-side under an IAM role. Resume text is the most sensitive user input and will be covered explicitly in the Week 5 privacy review, including how much of it is forwarded to the external AI provider and whether the provider retains it.
